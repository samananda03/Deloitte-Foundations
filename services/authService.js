...
const register = async (userData) => {
  try {
    // Ensure userData contains {username, password, role} if role is required during registration
...
const login = async (credentials) => {
  try {
    ...
    if (response.data.token) {
      // Save token, username, and role in localStorage for authenticated user
